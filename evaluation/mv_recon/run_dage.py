        ...
        for name_data, dataset in datasets_all.items():
            ...
            for data_idx in tqdm(range(len(dataset))):
                ...
                with torch.cuda.amp.autocast(enabled=True, dtype=torch.bfloat16):
                    pose_output = predictor.infer(video, lr_max_size=252, hr_num_tokens=None, hr_resolution_level=9)
                    # pose_output = predictor.infer(video, max_size=518, num_tokens=None, get_lr_points=False, resolution_level=9)
                ...
                pred_pts = pose_output["global_points"].cpu().numpy()
                ...
                pred_save_dict = {
                    ...
                    "rgb": colors,
                    "extrinsics": pose_output['camera_poses'].cpu().numpy(),
                }

                np.save(os.path.join(save_path, f"{scene_id.replace('/', '_')}.npy"), pred_save_dict, allow_pickle=True)

                gt_save_dict = {
                    "pointmap_global": gt_pts,
                    "pointmap_mask": valid_mask,
                    "rgb": colors,
                    "extrinsics": gt_camera_poses,
                }
                np.save(os.path.join(save_path, f"{scene_id.replace('/', '_')}_gt.npy"), gt_save_dict, allow_pickle=True)
