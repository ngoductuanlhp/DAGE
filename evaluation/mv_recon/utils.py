...
def rigid_transform(X, Y):
    """
    Estimates the SE(3) rigid transformation between `X` and `Y` point sets.
    
    Estimates R and t such as R @ X + t ~ Y (no scale factor).
    This assumes the two point clouds are already at the same scale.
