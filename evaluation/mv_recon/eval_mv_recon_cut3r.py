...
def get_args_parser():
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="/home/colligo/dngo/projects/CUT3R/src/cut3r_512_dpt_4_64.pth",
        help="Checkpoint path or hub name.",
    )

    parser.add_argument(
        "--conf_thresh", type=float, default=1.0, help="confidence threshold"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
...
def main(args):
    ...
    # Load Cut3r model
    print(f"Loading Cut3r model from {checkpoint_path}...")
    ...
    with torch.no_grad():
        for name_data, dataset in datasets_all.items():
            ...
            for data_idx in tqdm(range(len(dataset))):
                ...
                # Prepare video for Cut3r
                video = []
                for view in batch:
                    img_tensor = view["img"]
                    video.append(img_tensor)
                video = torch.stack(video, dim=1)  # (B, T, C, H, W)

                with CUDATimer("Model forward", enabled=True, num_frames=video.shape[1]):
                    with torch.cuda.amp.autocast(enabled=True, dtype=torch.bfloat16):
                        output = predictor(video)

                # Extract predicted 3D points from Cut3r output
                # Cut3r outputs 'local_points' in camera coordinates and 'local_points_conf'
                local_points = output['local_points'][0]  # (T, H, W, 3)
                conf = output['local_points_conf'][0]  # (T, H, W)
                
                # Apply confidence threshold
                mask = conf > args.conf_thresh
                
                # Cut3r outputs local points, we need to handle coordinate transformation
                # For now, we'll treat local_points as the predicted 3D structure
                pred_pts = local_points.cpu().numpy()

                # Get ground truth points and valid mask from dataset
                ...
                # Combine with Cut3r's confidence mask
                valid_mask = valid_mask & mask.cpu().numpy()
                ...
                if valid_mask.sum() < 100:
                    print(f"Warning: Too few valid points ({valid_mask.sum()}) for scene {scene_id}, skipping...")
                    continue

                # Apply Sim(3) alignment (with scale) since Cut3r may not output metric scale
                # Use umeyama instead of rigid_transform
