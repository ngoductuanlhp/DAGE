...
from Unip.eval.moge.utils.geometry_torch import (
    weighted_mean, 
    mask_aware_nearest_resize,
    intrinsics_to_fov
)
from Unip.eval.moge.utils.alignment import (
...
from Unip.eval.moge.utils.tools import key_average, timeit
...
def compute_metrics_video(
    ...
    mask = gt['depth_mask'] # T H W
    # gt_depth = gt['depth']
    gt_points = gt['points'] # T H W 3

    height, width = mask.shape[-2:]

    if anchor_frame == "all":
        stride = 48
        _, lr_mask, lr_index = mask_aware_nearest_resize(None, mask[::stride], (64, 64), return_index=True)
    else:
    ...
    if pred_points_affine_invariant is not None:
        ...
        # NOTE use anchor frame for alignment
        if anchor_frame == "all":

            # breakpoint()
            pred_points_lr_masked, gt_points_lr_masked = pred_points[::stride][lr_index][lr_mask], gt_points[::stride][lr_index][lr_mask]
            scale, shift = align_points_scale_xyz_shift(pred_points_lr_masked, gt_points_lr_masked, 1 / gt_points_lr_masked.norm(dim=-1))

            # breakpoint()

        else:
            pred_points_lr_masked, gt_points_lr_masked = pred_points[anchor_frame][lr_index][lr_mask], gt_points[anchor_frame][lr_index][lr_mask]
            scale, shift = align_points_scale_xyz_shift(pred_points_lr_masked, gt_points_lr_masked, 1 / gt_points_lr_masked.norm(dim=-1))

        # breakpoint()
        pred_points = pred_points * scale[None, None, None, None, ...] + shift[None, None, None, ...]
