...
def save_pointmap_as_ply(pointmap, rgb_image, output_path, far_threshold=20):
    # Convert pointmap to RGB image

    flat_pointmap = pointmap.reshape(-1, 3)

    valid = (flat_pointmap[:, 2] > 0) & (flat_pointmap[:, 2] <= far_threshold)

    flat_pointmap = flat_pointmap[valid]
    ...
    points_3d = flat_pointmap @ flip_transform
    ...
    # Debug: check how many points
    # print(f"Exporting {points_3d.shape[0]} points to {output_path}")

    # Export point cloud
    point_cloud = trimesh.PointCloud(points_3d, colors=colors)
