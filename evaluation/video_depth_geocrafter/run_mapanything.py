...
def prepare_frames_for_mapanything(frames_tensor, norm_type="dinov2"):
    """
    Convert video frames to MapAnything format.
    MapAnything expects list of dicts with:
    ...
    for t in range(T):
        img_tensor = frames_tensor[t]  # (3, H, W), already in [0, 1] range
...
def run_benchmark_inference_and_eval(
    ...
    with torch.no_grad(), tqdm(total=len(samples), desc=f"Processing {benchmark_name}", leave=False) as pbar:
        ...
        for i, sample in enumerate(samples):
            ...
            # Prepare frames for MapAnything
            views = prepare_frames_for_mapanything(video_tensor.squeeze(0)  , norm_type="dinov2")

            with CUDATimer("Model forward", enabled=True, num_frames=len(views)) as runtime:
            ...
            # Note: MapAnything produces absolute world coordinates for each frame independently
            # This may not be suitable for video depth evaluation which expects consistent coordinates
            pred_pts_list = []
            pred_masks = []

            for pred in predictions:
                pts3d = pred["pts3d_cam"]  # (1, H, W, 3) - absolute world coordinates
                pred_pts_list.append(pts3d[0])  # (H, W, 3)

                # Create a mask - MapAnything doesn't provide confidence masks by default
                # We'll assume all points are valid
                H, W = pts3d.shape[1:3]
                mask = torch.ones(H, W, dtype=torch.bool, device=device)
                pred_masks.append(mask)

            pred = torch.stack(pred_pts_list, dim=0)  # T H W 3
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run MapAnything inference and evaluation on a single benchmark in memory."
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    predictor = MapAnything.from_pretrained(checkpoint_path).to(device).eval()
    print("MapAnything model loaded successfully!")
