...
from third_party.moge import MoGe
    ...
    with torch.no_grad(), tqdm(total=len(samples), desc=f"Processing {benchmark_name}", leave=False) as pbar:
        ...
        for i, sample in enumerate(samples):
            ...
            with CUDATimer("Model forward", enabled=True, num_frames=video_tensor.shape[1], measure_memory=True) as runtime:
                ...
                #     output = predictor.infer(video_tensor, max_size=prior_max_size, num_tokens=None, get_lr_points=False, resolution_level=9)
                
                # force_width = 952
                # force_height = force_width * original_height / original_width
                # force_num_tokens = force_width//14 * force_height//14
                with torch.amp.autocast(enabled=True, dtype=torch.bfloat16, device_type='cuda'):
                    pred, pred_mask = predictor.forward_video(
                        video_tensor,
