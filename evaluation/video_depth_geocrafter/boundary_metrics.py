...
def fgbg_depth(
    ...
    """
    right_is_big_enough = (d[..., :, :-1] > 0.0) & ((d[..., :, 1:] / d[..., :, :-1]) > t)
    left_is_big_enough = (d[..., :, 1:] > 0.0) & ((d[..., :, :-1] / d[..., :, 1:]) > t)
    bottom_is_big_enough = (d[..., 1:, :] > 0.0) & ((d[..., 1:, :] / d[..., :-1, :]) > t)
    top_is_big_enough = (d[..., :-1, :] > 0.0) & ((d[..., :-1, :] / d[..., 1:, :]) > t)
    return (
        left_is_big_enough,
        top_is_big_enough,
        right_is_big_enough,
...
def boundary_f1(
    ...
    ----
        ...
        return_edge_maps (bool, optional): If True, return tuple of (pred_edge_map, gt_edge_map). Defaults to False.

    Returns:
    -------
        float or Tuple[np.ndarray, np.ndarray]: Boundary F1 score, or precision, or recall, or edge maps depending on the flags.
    ...
    if return_edge_maps:
        # Create edge maps by combining directional edges
        # ap, cp have shape (H, W-1), bp, dp have shape (H-1, W)
        H, W = pr.shape
        pred_edge = np.zeros((H, W), dtype=bool)
        gt_edge = np.zeros((H, W), dtype=bool)
        
        # Left and right edges (H, W-1) -> pad to (H, W)
        pred_edge[:, :-1] |= ap
        pred_edge[:, 1:] |= cp
        gt_edge[:, :-1] |= ag
        gt_edge[:, 1:] |= cg
        
        # Top and bottom edges (H-1, W) -> pad to (H, W)
        pred_edge[:-1, :] |= bp
        pred_edge[1:, :] |= dp
        gt_edge[:-1, :] |= bg
        gt_edge[1:, :] |= dg
...
def compute_boundary_metrics(
    ...
    alpha_threshold: float = 0.1,
    return_edge_maps: bool = False,
    edge_map_threshold: float = None,
) -> dict:
    """Compute boundary metrics for predicted and target depth maps.
    
    Args:
    ----
        predicted_depth (torch.Tensor): Predicted depth maps of shape (num_frames, H, W).
        target_depth (torch.Tensor): Target depth maps of shape (num_frames, H, W).
        t_min (float, optional): Minimum threshold. Defaults to 1.05.
        t_max (float, optional): Maximum threshold. Defaults to 1.25.
        N (int, optional): Number of thresholds. Defaults to 10.
        alpha_threshold (float, optional): Threshold for alpha masking. Defaults to 0.1.
        return_edge_maps (bool, optional): If True, return edge maps. Defaults to False.
        edge_map_threshold (float, optional): Specific threshold for edge map computation.
            If None, uses the middle threshold from the range. Defaults to None.
    
    Returns:
    -------
        dict: Dictionary containing boundary_f1 score and optionally edge maps.
    
    """
    num_frames = predicted_depth.shape[0]
    ...
    if return_edge_maps:
        pred_edge_maps = []
        gt_edge_maps = []
        threshold = edge_map_threshold if edge_map_threshold is not None else (t_min + t_max) / 2
        for t in range(num_frames):
            pred_edge, gt_edge = boundary_f1(
                invert_depth(predicted_depth[t]),
                invert_depth(target_depth[t]),
                threshold,
                return_edge_maps=True,
            )
            pred_edge_maps.append(pred_edge)
            gt_edge_maps.append(gt_edge)

        output = {}
        output["pred_edge_maps"] = np.stack(pred_edge_maps, axis=0)
        output["gt_edge_maps"] = np.stack(gt_edge_maps, axis=0)
        return output

    f1_scores = 0.0
    
    for t in range(num_frames):
        f1_score = SI_boundary_F1(predicted_depth[t], target_depth[t], t_min, t_max, N)
        f1_scores += f1_score
        
    f1_scores /= num_frames

    output = {
        "boundary_f1": f1_scores
    }
