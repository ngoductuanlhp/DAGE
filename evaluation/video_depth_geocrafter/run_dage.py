...
def run_benchmark_inference_and_eval(
    ...
    with torch.no_grad(), tqdm(total=len(samples), desc=f"Processing {benchmark_name}", leave=False) as pbar:
        ...
        for i, sample in enumerate(samples):
            ...
            # print(max(original_height, original_width)//14*14)
            video_tensor = frames_tensor.unsqueeze(0) # 1 T 3 H W
            with CUDATimer("Model forward", enabled=True, num_frames=video_tensor.shape[1]) as runtime:
                # output = predictor(video_tensor, device, align=False, prior_max_size=prior_max_size)
                with torch.cuda.amp.autocast(enabled=True, dtype=torch.bfloat16):
                    # if benchmark_name in ["unreal4k_2k", "urbansyn"]:
                    #     # prior_video_tensor = F.interpolate(frames_tensor, (original_height//2, original_width//2), mode='bilinear', antialias=True).clamp(0, 1)
                    #     original_aspect_ratio = original_width / original_height
                    #     prior_width = prior_max_size // 14 * 14
                    #     prior_height = int((prior_width / original_aspect_ratio) // 14 * 14)
                    #     prior_video_tensor = F.interpolate(
                    #         rearrange(video_tensor, 'b t c h w -> (b t) c h w'), (prior_height, prior_width), mode='bilinear', antialias=True
                    #     ).clamp(0, 1)
                    #     prior_video_tensor = rearrange(prior_video_tensor, '(b t) c h w -> b t c h w', t=frames_tensor.shape[0])
                    #     output = predictor.forward_long(video_tensor, prior_video_tensor, num_tokens=original_height//14 * original_width//14, force_chunk_size=4)

                    #     pred = torch.cat(output['local_points'], dim=1)[0] # T H W C
                    #     # pred_mask = torch.cat(output['mask'], dim=1)[0]
                    # else:
                    output = predictor.infer(video_tensor, lr_max_size=lr_max_size, hr_num_tokens=3600)
            ...
            if benchmark_name in ["sintel", "urbansyn", "unreal4k_quad", "monkaa", "unreal4k_2k"]:
                ...
                # results = compute_boundary_metrics(
                #     pred[..., 2],
                #     gt_pmap[..., 2],
                #     t_min=1.05,
                #     t_max=1.25,
                #     N=10,
                #     return_edge_maps=True,
                # )
                # pred_edge_maps = results['pred_edge_maps']
                # gt_edge_maps = results['gt_edge_maps']
                
                # # Convert binary edge maps to grayscale images (0-255) and save to disk
                # edge_maps_dir = os.path.join(benchmark_output_dir, "edge_maps")
                # os.makedirs(edge_maps_dir, exist_ok=True)
                
                # # Get sample name from video path
                # # sample_name = os.path.splitext(os.path.basename(video_path))[0]
                # sample_name = os.path.dirname(video_path).split('/')[-1]
                # sample_edge_dir = os.path.join(edge_maps_dir, sample_name)
                # os.makedirs(sample_edge_dir, exist_ok=True)
                
                # # Save each frame's edge maps with RGB images
                # num_frames = pred_edge_maps.shape[0]
                # separator_width = 5  # Width of white separator bar
                
                # # Get RGB frames at original resolution
                # rgb_frames = frames.astype(np.uint8)  # T H W C, range 0-255
                
                # for frame_idx in range(num_frames):
                #     # Get RGB frame
                #     rgb_frame = rgb_frames[frame_idx]  # H W C
                    
                #     # Convert boolean edge maps to uint8: False->0, True->255
                #     pred_edge_gray = (pred_edge_maps[frame_idx] * 255).astype(np.uint8)
                #     gt_edge_gray = (gt_edge_maps[frame_idx] * 255).astype(np.uint8)
                    
                #     # Resize RGB frame if edge maps have different dimensions
                #     edge_height, edge_width = pred_edge_gray.shape
                #     if rgb_frame.shape[0] != edge_height or rgb_frame.shape[1] != edge_width:
                #         rgb_frame_resized = np.array(Image.fromarray(rgb_frame).resize(
                #             (edge_width, edge_height), Image.BILINEAR
                #         ))
                #     else:
                #         rgb_frame_resized = rgb_frame
                    
                #     # Convert edge maps to RGB for concatenation
                #     pred_edge_rgb = np.stack([pred_edge_gray] * 3, axis=-1)  # H W 3
                #     gt_edge_rgb = np.stack([gt_edge_gray] * 3, axis=-1)  # H W 3
                    
                #     # Create white separator bar (RGB)
                #     separator = np.ones((edge_height, separator_width, 3), dtype=np.uint8) * 255
                    
                #     # Horizontally concatenate: RGB | white bar | pred edge | white bar | gt edge
                #     concatenated = np.concatenate([
                #         rgb_frame_resized, separator, pred_edge_rgb, separator, gt_edge_rgb
                #     ], axis=1)
                    
                #     # Save concatenated image
                #     concat_img = Image.fromarray(concatenated, mode='RGB')
                #     concat_path = os.path.join(sample_edge_dir, f"frame_{frame_idx:04d}_edge_comparison.png")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run pointmap VAE inference and evaluation on a single benchmark in memory."
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    print(f"run with lr_max_size: {lr_max_size}")
    ...
    # -------------------- Preparation --------------------
    logging.info(
        f"Processing settings: checkpoint = `{checkpoint_path}`, "
