...
def run_benchmark_inference_and_eval(
    ...
    with torch.no_grad(), tqdm(total=len(samples), desc=f"Processing {benchmark_name}", leave=False) as pbar:
        ...
        for i, sample in enumerate(samples):
            ...
            inference_height, inference_width = video_tensor.shape[-2:]
            ...
            if max_size is None:
                max_size = max(inference_height // patch_size * patch_size, inference_width // patch_size * patch_size)
            ...
            if inference_height != new_height or inference_width != new_width:
                ...
                video_tensor = F.interpolate(video_tensor.squeeze(0), (new_height, new_width), mode='bilinear', antialias=True).clamp(0, 1).unsqueeze(0)

            with CUDATimer("Model forward", enabled=True, num_frames=video_tensor.shape[1], measure_memory=True) as runtime:
                with torch.amp.autocast(enabled=True, dtype=torch.bfloat16, device_type='cuda'):
                    # Aggregate tokens
                    aggregated_tokens_list, ps_idx = predictor.aggregator(video_tensor)
                    
                    # Predict Cameras
                    pose_enc = predictor.camera_head(aggregated_tokens_list)[-1]
                    
                    # Predict Depth Maps
                    depth_map, depth_conf = predictor.depth_head(aggregated_tokens_list, video_tensor, ps_idx)
                
            # Convert to float32 for unprojection
            ...
            pred_mask = depth_conf >= 1.0
            
            pred = local_points  # T H W C
            
            # Interpolate back to original resolution if needed
            if inference_height != new_height or inference_width != new_width:
                logging.debug(f"Interpolating outputs back to ({inference_height}, {inference_width})")
                pred = F.interpolate(
                    pred.permute(0, 3, 1, 2),  # (T, 3, H, W)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run VGGT inference and evaluation on a single benchmark in memory."
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    logging.info(
        ...
        f"benchmark = {benchmark_name}, max_size = {max_size}, seed = {seed}"
    )
