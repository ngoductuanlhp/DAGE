...
def run_benchmark_inference_and_eval(
    ...
    with torch.no_grad(), tqdm(total=len(samples), desc=f"Processing {benchmark_name}", leave=False) as pbar:
        ...
        for i, sample in enumerate(samples):
            ...
            with CUDATimer("Model forward", enabled=True, num_frames=len(vid), measure_memory=True) as runtime:

                if benchmark_name in ["urbansyn", "unreal4k_2k"]:
                    original_aspect_ratio = original_width / original_height
                    target_width = original_width // 14 * 14
                    # target_width = 1800
                    target_height = int((target_width / original_aspect_ratio) // 14 * 14)
                    ...
                    resized_video_tensor = torch.from_numpy(np.stack(resized_video, axis=0)).float().permute(0, 3, 1, 2).to(device)[None] / 255.0
                    
                    with torch.amp.autocast(device_type='cuda', enabled=True, dtype=torch.bfloat16):
                        output = predictor.forward_long(resized_video_tensor, (original_height, original_width))
                else:
                    ...
                    output = predictor.infer(video_tensor, max_size=518)


                # output = predictor.infer(video_tensor, max_size=252)
                # output = predictor.infer(video_tensor, max_size=None)
                # output = predictor(
                #     imgs=video_tensor,
                #     prior_max_size=252,
                # )
            
            # pred = output['local_points'][0] # T H W C
            # pred_mask = torch.sigmoid(output["conf"][0]).squeeze(-1) > 0.2
            ...
            if benchmark_name in ["sintel", "urbansyn", "unreal4k_quad", "monkaa", "unreal4k_2k"]:
                ...
                # # Save each frame's predicted edge maps
                ...
                #     # Save predicted edge map
                #     pred_edge_img = Image.fromarray(pred_edge_gray, mode='L')
                #     pred_edge_path = os.path.join(sample_edge_dir, f"frame_{frame_idx:04d}_pred_edge.png")
                #     pred_edge_img.save(pred_edge_path)
...
if __name__ == "__main__":
    ...
    predictor = Pi3Teacher.from_pretrained(pretrained_model_name_or_path=checkpoint_path, strict=True).to(device).eval()
    # predictor = Pi3V4.from_pretrained(pretrained_model_name_or_path=checkpoint_path, strict=True).to(device).eval()
