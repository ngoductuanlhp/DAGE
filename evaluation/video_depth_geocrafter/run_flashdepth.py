...
from third_party.flashdepth.model import FlashDepth
...
def depth_to_pointmap(depth, intrinsics):
    """
    Convert depth map(s) to point map(s) using camera intrinsics.

    Args:
        depth: (H, W) or (T, H, W) depth map(s)
        intrinsics: camera intrinsics dict with 'fx', 'fy', 'cx', 'cy'

    Returns:
        point_map: (H, W, 3) or (T, H, W, 3) point map(s)
    """
    if depth.ndim == 2:
        # Single frame: (H, W)
        T, H, W = 1, depth.shape[0], depth.shape[1]
        depth = depth.unsqueeze(0)  # Add temporal dimension
    elif depth.ndim == 3:
        # Video: (T, H, W)
        T, H, W = depth.shape
    else:
        raise ValueError(f"Depth must be 2D (H, W) or 3D (T, H, W), got {depth.ndim}D")

    fx, fy = intrinsics['fx'], intrinsics['fy']
    cx, cy = intrinsics['cx'], intrinsics['cy']

    # Create pixel grid (shared across all frames)
    y, x = torch.meshgrid(
    ...
    # Unproject to 3D for all frames
    z = depth  # (T, H, W)
    x_3d = (x - cx) * z / fx  # (T, H, W)
    y_3d = (y - cy) * z / fy  # (T, H, W)

    point_map = torch.stack([x_3d, y_3d, z], dim=-1)  # (T, H, W, 3)

    # Return single frame format if input was single frame
    if T == 1:
        point_map = point_map.squeeze(0)  # (H, W, 3)
        ...
        for i, sample in enumerate(samples):
            ...
            with CUDATimer("Model forward", enabled=True, num_frames=video_tensor.shape[1]) as runtime:
                with torch.amp.autocast(enabled=True, dtype=torch.bfloat16, device_type='cuda'):
                    output = predictor.infer(video_tensor, skip_mamba=skip_mamba)
                    # output = predictor.infer(video_tensor, max_size=max(original_height, original_width))
            
            # pred = output['local_points'] # T H W C
            pred_depth = output['depth'] # T H W
            pred_mask = output['mask']

            cx, cy = original_width / 2, original_height / 2
            fx = fy = original_width
            intrinsics = {'fx': fx, 'fy': fy, 'cx': cx, 'cy': cy}
            
            # Convert depth to point map
            pred = depth_to_pointmap(pred_depth, intrinsics)  # H W 3
            pred = pred.to(device)
            ...
            # Ensure dtype consistency for metrics computation
            pred = pred.float()  # Convert from bfloat16 to float32 to match gt_pmap
            ...
            if benchmark_name in ["sintel", "urbansyn", "unreal4k_quad", "monkaa"]:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--skip_mamba", type=bool, default=False, help="Skip mamba."
    )
    ...
    # predictor = prior_model = Cut3r.from_pretrained("/home/colligo/dngo/projects/CUT3R/src/cut3r_512_dpt_4_64.pth", strict=False).to(device).eval()
    flash_depth_config = OmegaConf.load('third_party/flashdepth/configs/flashdepth-l/config.yaml')
    predictor = FlashDepth(**dict(
        batch_size=flash_depth_config.training.batch_size,
        hybrid_configs=flash_depth_config.hybrid_configs,
        training=not flash_depth_config.inference,
        **flash_depth_config.model,
    ))
    checkpoint = torch.load("checkpoints/flashdepth_l.pth", map_location='cpu')
