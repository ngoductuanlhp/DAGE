...
def load_mp3d_traj(gt_cam_dir):
    """Load MP3D trajectory from .npz camera files.
    Each .npz file contains 'pose' (4x4 cam2world matrix) and 'intrinsics'.
    Similar to sintel, poses are already in cam2world format.
    """
    
    # Get all .npz files in the cam directory
    cam_files = sorted(glob.glob(os.path.join(gt_cam_dir, "*.npz")))
    ...
    for i, cam_file in enumerate(cam_files):
        ...
        gt_pose = cam_data['pose']  # 4x4 cam2world matrix (already in cam2world like sintel after inversion)
        ...
        xyzw = R.as_quat()  # scalar-last for scipy
        wxyz = np.array([xyzw[-1], xyzw[0], xyzw[1], xyzw[2]])  # convert to wxyz format
        wxyzs.append(wxyz)
    ...
    tum_gt_poses = np.stack(tum_gt_poses, 0)
    # Optionally center trajectory (uncomment if needed, like sintel does)
...
def load_traj(gt_traj_file, traj_format="sintel", skip=0, stride=1, num_frames=None):
    ...
    if traj_format == "replica":
        ...
        traj_tum = np.column_stack((xyz, quat))
    elif traj_format == "mp3d":
        traj_tum, timestamps_mat = load_mp3d_traj(gt_traj_file)
    else:
