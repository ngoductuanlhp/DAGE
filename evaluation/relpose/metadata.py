...
dataset_metadata = {
    ...
    "bonn": {
        "img_path": "/mnt/localssd/pose_eval/bonn/rgbd_bonn_dataset",
        "mask_path": None,
    ...
    "scannet": {
        "img_path": "/mnt/localssd/pose_eval/scannetv2",
        "mask_path": None,
    ...
    "scannet-33": {
        "img_path": "/mnt/localssd/pose_eval/scannetv2_3_33",
        "mask_path": None,
        "dir_path_func": lambda img_path, seq: os.path.join(img_path, seq, "color_90"),
        "gt_traj_func": lambda img_path, anno_path, seq: os.path.join(
            img_path, seq, "pose_90.txt"
    ...
    "tum": {
        "img_path": "/mnt/localssd/pose_eval/tum",
        "mask_path": None,
    ...
    "sintel": {
        "img_path": "/mnt/localssd/pose_eval/sintel/training/final",
        "anno_path": "/mnt/localssd/pose_eval/sintel/training/camdata_left",
        "mask_path": None,
        "dir_path_func": lambda img_path, seq: os.path.join(img_path, seq),
    ...
    "mp3d": {
        "img_path": "/mnt/localssd/pose_eval/mp3d",
        "mask_path": None,
        ...
        "traj_format": "mp3d",
        "seq_list": ['17DRP5sb8fy', '2n8kARJN3HM', '759xd9YjKW5', 'B6ByNegPMKs', 'HxpKQynjfin', 'Pm6F8kyY3z2', 'SN83YJsR3w2', 'V2XKFyX4ASd', 'Vvot9Ly1tCj', 'YFuZgdQ5vWj', 'ac26ZMwG7aT', 'fzynW3qQPVF', 'jh4fc5c5qoQ', 'p5wJjkQkbXX', 'qoiz87JEwZ2', 's8pcmisQ38h', 'vyrNrziPKCB', 'yqstnuAEVhm'],
        "full_seq": False,
        "mask_path_seq_func": lambda mask_path, seq: None,
        "skip_condition": None,
        "process_func": lambda args, img_path: process_mp3d(args, img_path),
    },

    "mp3d_short": {
        "img_path": "/mnt/localssd/pose_eval/mp3d_short",
        "mask_path": None,
        "dir_path_func": lambda img_path, seq: os.path.join(img_path, seq, "rgb"),
        "gt_traj_func": lambda img_path, anno_path, seq: os.path.join(
            img_path, seq, "cam"
...
def process_mp3d(args, img_path):
    # Read valid scenes list if it exists
    valid_scenes_file = os.path.join(img_path, "valid_scenes.txt")
    if os.path.exists(valid_scenes_file):
        with open(valid_scenes_file, 'r') as f:
            seq_list = [line.strip() for line in f if line.strip()]
    else:
        # Fall back to listing all directories
        seq_list = sorted([d for d in os.listdir(img_path) 
                          if os.path.isdir(os.path.join(img_path, d))])

        seq_list = seq_list[:10]
    
    for seq in tqdm(seq_list):
        filelist = sorted(glob.glob(f"{img_path}/{seq}/rgb/*.png"))
        save_dir = f"{args.output_dir}/{seq}"
        yield filelist, save_dir
