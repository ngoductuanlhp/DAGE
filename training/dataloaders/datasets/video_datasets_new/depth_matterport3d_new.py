...
class VideoDepthMatterport3DNew(VideoDepthHabitat3DNew):
    """
    Video sequence dataset for Matterport3D data.
    Inherits from Habitat3D dataset as they share identical structure.
    Simply override the dataset_config_key to use Matterport3D configuration.
    """
    depth_type = "sfm"  # Matterport3D has metric depth data
    dataset_config_key = "matterport3d"  # Override to use Matterport3D config


def generate_parquet_file(matterport3d_data_dir, output_parquet_filename="matterport3d.parquet"):
    """
    Generate parquet file for Matterport3D dataset.
    
    Args:
        matterport3d_data_dir: Root directory containing the processed Matterport3D data
                              Expected structure: matterport3d_data_dir/{scene_id:08}/rgb/view{idx:05d}.png
    ...
    scene_dirs = sorted([d for d in os.listdir(matterport3d_data_dir) 
                        if os.path.isdir(os.path.join(matterport3d_data_dir, d))])
    
    print(f"Found {len(scene_dirs)} scene directories for Matterport3D")
    
    for scene_id in tqdm.tqdm(scene_dirs):
        scene_path = os.path.join(matterport3d_data_dir, scene_id)
        ...
        for subscene_id in subscene_dirs:
            ...
            # Create paths
            data_prefix = DATASET_CONFIG["matterport3d"]["prefix"] if "matterport3d" in DATASET_CONFIG else "matterport3d"
...
if __name__ == "__main__":
    ...
    # matterport3d_data_dir = "/mnt/localssd/mp3d_release"
    # generate_parquet_file(matterport3d_data_dir)
    # quit()
    
    os.makedirs("data_vis", exist_ok=True)
    print("\nTesting Matterport3D dataset...")

    dataset_name = "matterport3d_new"
    save_dir = f"data_vis/{dataset_name}"
    os.makedirs(save_dir, exist_ok=True)
    
    resolutions = [[0, 1]] * 10000
    habitat_dataset = VideoDepthMatterport3DNew(
        T=10, 
        stride_range=(1, 5),
        transform=True,
