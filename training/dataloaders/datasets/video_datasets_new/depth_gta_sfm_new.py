...
class SynchronizedTransform_GTASFMSVideo:
...
class VideoDepthGTASFMSNew(Dataset):
    ...
    def __init__(
        ...
        parquet_data = download_file(
            ...
            DATASET_CONFIG["gta_sfm"]["parquet_path"],
        )
        ...
        if self.use_moge:
            ...
            self.transform = (
                SynchronizedTransform_GTASFMSVideo(H=resize[0], W=resize[1])
                if transform
                else None
            )
    ...
    def _organize_by_video(self):
        ...
        if not video_frames:
            raise RuntimeError("No valid GTA-SFM entries found in parquet")
    ...
    def _load_single_frame(self, frame_data):
        try:
            ...
            buffer = np.frombuffer(depth_bytes, np.uint8)
            del depth_bytes  # Free memory immediately
            
            depth_array = cv2.imdecode(
                buffer, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH
            )
            del buffer  # Free memory immediately
            
            if depth_array is None:
                raise IOError(f"Failed to decode depth map: {frame_data['depth_path']}")
            ...
            print(f"[gta_sfm_video] Error loading frame: {str(e)}")
    ...
    def __getitem__(self, idx):
        try:
            ...
            if self.use_cut3r_frame_sampling:
                ...
                is_video = True
                initial_stride = random.randint(
                    self.stride_range[0], self.stride_range[1]
                )
                stride = initial_stride

                if not self._can_sample_sequence(
                    video_name, start_idx, stride, num_frames=num_frames
                ):
                    is_success = False
                    while stride > max(self.stride_range[0], 1):
                        stride -= 1
                        if self._can_sample_sequence(
                            video_name, start_idx, stride, num_frames=num_frames
                        ):
                            is_success = True
                            frame_positions = [
                                min(start_idx + i * stride, len(frames) - 1)
                                for i in range(num_frames)
                            ]
                            break
                else:
            ...
            print(f"[gta_sfm_video] Error processing sample: {str(e)}")
            if num_frames is None:
                num_frames = self.T
...
def generate_parquet_file(
    processed_root: str = "/mnt/localssd/GTA-sfm-processed",
    split: str = "train",
    parquet_local: str | None = None,
):
    """Generate parquet index for GTA-SFM processed data."""
    ...
    if not processed_root_path.is_dir():
        raise FileNotFoundError(
            f"Processed GTA-SFM root '{processed_root}' does not exist"
        )

    cfg = DATASET_CONFIG["gta_sfm"]
    prefix = Path(cfg["prefix"])
    parquet_key = cfg["parquet_path"]

    records = []
    for seq_dir in tqdm.tqdm(
        sorted(processed_root_path.iterdir()), desc="Scanning GTA-SFM sequences"
    ):
        ...
        for rgb_file in sorted(rgb_dir.glob("*.jpg")):
            ...
            depth_file = depth_dir / f"{frame_number}.exr"
            cam_file = cam_dir / f"{frame_number}.npz"
            if not (depth_file.is_file() and cam_file.is_file()):
                continue

            cam_data = np.load(cam_file)
            intrinsics = cam_data.get("intrinsics")
            extrinsics = cam_data.get("cam2world")
    ...
    if not records:
        raise RuntimeError("No GTA-SFM records were found to build the parquet file")
    ...
    parquet_filename = parquet_local or f"gta_sfm_{split}.parquet"
    df.to_parquet(parquet_filename)
    print(f"[gta_sfm] Generated parquet with {len(df)} entries -> {parquet_filename}")

    upload_file(parquet_filename, parquet_key)
    print(f"[gta_sfm] Saved parquet to {parquet_key}")
...
if __name__ == "__main__":
    ...
    print("\nTesting gta_sfm video dataset with new sampling...")

    dataset_name = "gta_sfm_new"
    save_dir = f"data_vis/{dataset_name}"
    os.makedirs(save_dir, exist_ok=True)
    
    resolutions = [[0,1]] * 10000
    video_dataset = VideoDepthGTASFMSNew(
        T=10, 
        stride_range=(1, 5),
        transform=True,
