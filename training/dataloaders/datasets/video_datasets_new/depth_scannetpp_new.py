...
# Original dataset parameters (from preprocessing script)
OG_SCANNETPP_TARGET_RESOLUTION = 920


class SynchronizedTransform_ScanNetPPVideo:
    ...
    def __call__(self, rgb_images, depth_images, intrinsics_list, extrinsics_list=None):
        ...
        # print(rgb_tensors[0].shape, depth_tensors[0].shape, intrinsics_tensors[0].shape, extrinsics_tensors[0].shape, flip)
...
class VideoDepthScanNetPPNew(Dataset):
    depth_type = "sfm"  # ScanNetPP uses real depth from mesh rendering
    is_metric_scale = True
    # NOTE original resolution is (1035, 690)
    
    def __init__(self, T=5, stride_range=(1, 3), transform=True, resize=(360, 540), near_plane=0.05, far_plane=65.0, resolutions=None, use_moge=False, moge_augmentation=None,
        use_cut3r_frame_sampling=False, video_prob=0.6, fix_interval_prob=0.6, block_shuffle=16
    ):
        """
        Video sequence dataset for ScanNet++ depth data.
        ...
        Args:
            ...
            near_plane (float): Near clipping plane for depth (from preprocessing: 0.05)
            far_plane (float): Far clipping plane for depth (from preprocessing: 20.0)
            resolutions (list): List of resolutions for multi-scale training
        """
        parquet_data = download_file(DATASET_CONFIG["scannetpp"]["parquet_path"])
        ...
        if self.use_moge:
            ...
            self.transform = SynchronizedTransform_ScanNetPPVideo(H=resize[0], W=resize[1]) if transform else None
    ...
    def _can_sample_sequence(self, scene_name, start_idx, stride, num_frames=None):
        ...
        # For ScanNetPP, we can sample any frames within the scene as they're not necessarily sequential
        # But we still check if we have enough frames
        return True

    def _load_single_frame(self, frame_data):
        """Load a single frame's RGB, depth, and intrinsics for ScanNetPP."""
        try:
            # Load RGB
            ...
            # Load depth (ScanNetPP specific format - PNG 16-bit scaled by 1000)
            ...
            # print(rgb_image.size, depth_image.size)
            ...
            print(f"[scannetpp_video] Error loading frame: {str(e)}")
    ...
    def __getitem__(self, idx):
        try:
            ...
            # print(f"original frames: {len(frames)}, start_idx: {start_idx}, num_frames: {num_frames}, stride_range: {self.stride_range}, frame positions: {frame_positions}")
            
            # print(f"ScanNetPP data, scene name: {scene_name}, frame positions: {frame_positions}, {self.use_cut3r_frame_sampling}")
            ...
            for frame_idx in frame_positions:
                ...
                if valid:
                    ...
                    if rgb_images:  # Use previous frame as fallback
                        ...
                        rgb_images.append(Image.new('RGB', (920, 690)))
                        depth_images.append(Image.fromarray(np.zeros((690, 920), dtype=np.float32)))
            ...
            for t in range(num_frames):
                ...
                inf_depth_mask = torch.zeros_like(depth_tensor).bool() # No inf depth mask for ScanNetPP
            ...
            print(f"[scannetpp_video] Error processing frame: {str(e)}")
...
def generate_parquet_file():
    """
    Generate parquet file from preprocessed ScanNetPP data.
    Works directly with individual scene metadata files since the consolidation step isn't run.
    """
    import tqdm
    import glob
    
    # Path to preprocessed ScanNetPP data
    DATA_DIR = "<PATH TO SCANNETPP RAW DATA>"
    data_path = DATASET_CONFIG["scannetpp"]["prefix"]
    
    # Find all scene directories
    scene_dirs = [d for d in os.listdir(DATA_DIR) if os.path.isdir(os.path.join(DATA_DIR, d)) and d != "__pycache__"]
    print(f"Found {len(scene_dirs)} scenes: {scene_dirs}")
    
    # Create empty list to accumulate records (much more efficient than pd.concat in loop)
    df_columns = ['rgb_path', 'depth_path', 'intrinsics', 'extrinsics', 'scene_name', 'frame_number']
    records = []
    
    print("Converting ScanNetPP metadata to parquet format...")
    for scene_name in tqdm.tqdm(scene_dirs):
        scene_metadata_path = os.path.join(DATA_DIR, scene_name, "scene_metadata.npz")
        
        if not os.path.exists(scene_metadata_path):
            print(f"Warning: No metadata found for scene {scene_name}, skipping...")
            continue
        ...
        # Filter to only DSC images first
        selected_images = [images[i] for i in selected_indices]
        # selected_trajectories = [trajectories[i] for i in selected_indices]
        # selected_intrinsics = [intrinsics[i] for i in selected_indices]
        
        # Sort alphabetically by image name and get sorted indices
        sorted_indices = sorted(range(len(selected_images)), key=lambda i: selected_images[i])
        
        # breakpoint()
        # Apply sorting to all arrays
        images = [selected_images[i] for i in sorted_indices]
        trajectories = [trajectories[i] for i in sorted_indices]
        intrinsics = [intrinsics[i] for i in sorted_indices]

        # breakpoint()
        
        assert len(images) == len(trajectories) == len(intrinsics)
        ...
        for i, image_name in enumerate(images):
            # Construct paths
            # breakpoint()
            rgb_filename = image_name + ".jpg"
            depth_filename = image_name + ".png"
            
            rgb_path = f"{data_path}/{scene_name}/images/{rgb_filename}"
            depth_path = f"{data_path}/{scene_name}/depth/{depth_filename}"
            
            # Get intrinsics and extrinsics for this frame
            intrinsics_matrix = intrinsics[i]
            extrinsics_matrix = trajectories[i]
            
            # Flatten matrices for parquet compatibility
            intrinsics_flat = intrinsics_matrix.flatten().tolist()
            extrinsics_flat = extrinsics_matrix.flatten().tolist()
            ...
            records.append({
                ...
                'scene_name': scene_name,
                'frame_number': i  # Frame number within the scene
            })
    ...
    # Save parquet file
    parquet_filename = "scannetpp_train.parquet"
    df_new.to_parquet(parquet_filename)
    
    # Read and verify
    df_verify = pd.read_parquet(parquet_filename)
    print(f"Generated parquet file with {len(df_verify)} entries across {len(scene_dirs)} scenes")
    
    # Save
    upload_file(parquet_filename, DATASET_CONFIG["scannetpp"]["parquet_path"])
...
if __name__ == "__main__":
    ...
    print("\nTesting ScanNetPP video dataset with new sampling...")

    dataset_name = "scannetpp_new"
    save_dir = f"data_vis/{dataset_name}"
    os.makedirs(save_dir, exist_ok=True)
    
    resolutions = [[0,1]] * 10000
    video_dataset = VideoDepthScanNetPPNew(
        T=10, 
        stride_range=(1, 5),
        transform=True,
