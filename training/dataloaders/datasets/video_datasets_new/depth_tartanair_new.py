...
OG_TARTANAIR_HEIGHT = 480
OG_TARTANAIR_WIDTH = 640
OG_TARTANAIR_FOCAL_LENGTH = 320
OG_TARTANAIR_INTRINSICS = np.array([
    [320, 0, 320],
    [0, 320, 240],
    [0, 0, 1]
]).astype(np.float32)

class SynchronizedTransform_TartanairVideo:
...
class VideoDepthTartanairNew(Dataset):

    depth_type = "synthetic"
    is_metric_scale = True
    # NOTE original resolution is 640x480
    def __init__(self, T=5, stride_range=(1, 3), transform=True, resize=(480, 640), near_plane=1e-5, far_plane=80.0, resolutions=None, use_moge=False, moge_augmentation=None,
        use_cut3r_frame_sampling=False, video_prob=0.6, fix_interval_prob=0.6, block_shuffle=16
    ):
        """
        Video sequence dataset for TartanAir depth data.
        ...
        """
        parquet_data = download_file(DATASET_CONFIG["tartanair"]["parquet_path"])
        ...
        # self.transform = SynchronizedTransform_TartanairVideo(H=resize[0], W=resize[1]) if transform else None
    ...
    def _load_single_frame(self, frame_data):
        """Load a single frame's RGB, depth, and intrinsics for TartanAir."""
        try:
            # Load RGB
            ...
            # Load depth (TartanAir specific format - numpy array)
            ...
            # Get intrinsics (fixed for TartanAir)
            intrinsics = OG_TARTANAIR_INTRINSICS.copy()
            ...
            print(f"[tartanair_video] Error loading frame: {str(e)}")
    ...
    def __getitem__(self, idx):
        try:
            ...
            # print(f"TartanAir data, scene name: {video_name}, frame positions: {frame_positions}, {self.use_cut3r_frame_sampling}")
            ...
            for frame_idx in frame_positions:
                ...
                if valid:
                    ...
                    if rgb_images:  # Use previous frame as fallback
                        ...
                        intrinsics_list.append(OG_TARTANAIR_INTRINSICS.copy())
            ...
            print(f"[tartanair_video] Error processing frame: {str(e)}")
...
def generate_parquet_file():
    ...
    DATA_DIR = "/mnt/localssd/tartanair"
    data_path =  DATASET_CONFIG["tartanair"]["prefix"]
    scenes = os.listdir(DATA_DIR)
    df_new = pd.DataFrame(columns=['rgb_path', 'depth_path', 'extrinsics', 'video_name', 'frame_number'])
    for scene in tqdm.tqdm(scenes):
        print(scene)
        shots = os.listdir(os.path.join(DATA_DIR, scene, 'Hard'))
        for shot in shots:
            rgb_dir = os.path.join(DATA_DIR, scene, 'Hard', shot, 'image_left')
            depth_dir = os.path.join(DATA_DIR, scene, 'Hard', shot, 'depth_left')
            pose_file = os.path.join(DATA_DIR, scene, 'Hard', shot, 'pose_left.txt')
            pose = np.loadtxt(pose_file)
            rgb_files = os.listdir(rgb_dir)
            for rgb_file in rgb_files:
                frame_number = int(rgb_file.split('_')[0])
                depth_file = rgb_file.replace('.png', '_depth.npy')
                ...
                extrinsics = xyzqxqyqxqw_to_c2w(pose[frame_number])
                # Flatten the 4x4 matrix to a 1D array for parquet compatibility
                extrinsics_flat = extrinsics.flatten().tolist()
                df_new = pd.concat([df_new, pd.DataFrame({
                    'rgb_path': [rgb_path],
                    'depth_path': [depth_path],
                    'extrinsics': [extrinsics_flat],
                    'video_name': [scene + '_' + shot],
                    'frame_number': [frame_number]
                })], ignore_index=True)

    df_new.to_parquet("tartanair_train.parquet")
    # read the parquet file
    df_new = pd.read_parquet("tartanair_train.parquet")
    # print the length of the dataframe
    print(len(df_new))
    # Save the parquet file
    upload_file("tartanair_train.parquet", DATASET_CONFIG["tartanair"]["parquet_path"])
    # remove the local file
    os.remove("tartanair_train.parquet")

def load_depth_video(video_path):
    """
    Load a previously saved depth MP4 video and convert it back to depth values.
    
    Args:
        video_path (str): Path to the depth MP4 file
        original_depth_range (tuple): Optional (min_depth, max_depth) to denormalize
                                    If None, depth will be in 0-255 range
    
    Returns:
        numpy.ndarray: Depth sequence of shape [T, H, W] with actual depth values
    """
    import cv2
    ...
    # Stack into numpy array [T, H, W]
    depth_sequence = np.stack(depth_frames, axis=0)
...
if __name__ == "__main__":
    ...
    dataset_name = "tartanair_new"
    save_dir = f"data_vis/{dataset_name}"
    os.makedirs(save_dir, exist_ok=True)
    
    resolutions = [[0,1]] * 10000
    video_dataset = VideoDepthTartanairNew(
        T=10, 
        stride_range=(1, 8),
        transform=True,
