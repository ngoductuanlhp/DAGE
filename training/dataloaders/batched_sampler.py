...
class CustomRandomSampler(Sampler):
    ...
    def __iter__(self):
        ...
        if self.number_of_resolutions is None:

            idxs = np.c_[sample_idxs, _view_idxs]
            ...
            if self.number_of_resolutions > 1:
                ...
                p = p / p.sum()
                _feat_idxs = rng.choice(self.number_of_resolutions, size=n_batches, p=p)
            else:
                _feat_idxs = rng.integers(self.number_of_resolutions, size=n_batches)
            _feat_idxs = np.broadcast_to(_feat_idxs[:, None], (n_batches, self.batch_size))
            _feat_idxs = _feat_idxs.ravel()[: self.total_size]


            idxs = np.c_[sample_idxs, _view_idxs, _feat_idxs]
...
def make_sampler(
    dataset,  batch_size, number_of_resolutions, min_num_frames, max_num_frames, shuffle=True, drop_last=True, process_index=0
):
    if not (shuffle):
