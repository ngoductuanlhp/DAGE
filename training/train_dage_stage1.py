...
from loss import Pi3Loss, Pi3DistillLoss
from util.lr_scheduler import IterStep, IterExponential, IterCosine
import time
import utils3d
...
def evaluate_mv_recon(model, device, logger):
    """
    Evaluate multi-view reconstruction on DTU dataset.
    ...
    Returns:
        dict: Evaluation metrics (acc, comp, nc1, nc2, acc_med, comp_med, nc1_med, nc2_med)
    """
    
    logger.info("=" * 80)
    logger.info("Running Multi-View Reconstruction Evaluation")
    logger.info("=" * 80)
    
    model.eval()
    
    # Create DTU dataset (subset for faster evaluation during training)
    dataset = NRGBD(
        split="test",
        ROOT="<PATH_TO_NRGBD>",
        resolution=(518, 518),
    ...
    num_evaluated = 0
    max_eval_samples = 5  # Limit number of samples during training for speed
    
    with torch.no_grad():
        for data_idx in tqdm(range(min(len(dataset), max_eval_samples)), total=min(len(dataset), max_eval_samples), desc="Evaluating multi-view reconstruction", leave=False):
            try:
                batch = default_collate([dataset[data_idx]])
                ignore_keys = set([
                    "depthmap", "dataset", "label", "instance", 
                    "idx", "true_shape", "rng"
                ])
                ...
                # Get valid mask for later use
                valid_mask = torch.stack([view["valid_mask"][0] for view in batch], dim=0)
                ...
                pred_pts = output['points'].cpu().numpy()
                ...
                colors = images.permute(0, 2, 3, 1)[valid_mask_np].cpu().numpy().reshape(-1, 3)
                
                # Coarse alignment using Umeyama
                c, R, t = umeyama(pred_pts[valid_mask_np].T, gt_pts[valid_mask_np].T)
                pred_pts = c * np.einsum('nhwj, ij -> nhwi', pred_pts, R) + t.T
                
                # Filter invalid points
                pred_pts_filtered = pred_pts[valid_mask_np].reshape(-1, 3)
                gt_pts_filtered = gt_pts[valid_mask_np].reshape(-1, 3)
                
                # Create point clouds
                pcd = o3d.geometry.PointCloud()
                pcd.points = o3d.utility.Vector3dVector(pred_pts_filtered)
                pcd.colors = o3d.utility.Vector3dVector(colors)
                
                pcd_gt = o3d.geometry.PointCloud()
                pcd_gt.points = o3d.utility.Vector3dVector(gt_pts_filtered)
                ...
                logger.warning(f"Error evaluating sample {data_idx}: {e}")
                continue
    
    # Calculate averages
    eval_dict = None
    if num_evaluated > 0:
        acc_all /= num_evaluated
        comp_all /= num_evaluated
        nc1_all /= num_evaluated
        nc2_all /= num_evaluated
        nc_all = (nc1_all + nc2_all) / 2
        acc_all_med /= num_evaluated
        comp_all_med /= num_evaluated
        nc1_all_med /= num_evaluated
        nc2_all_med /= num_evaluated
        nc_all_med = (nc1_all_med + nc2_all_med) / 2
        
        logger.info(f"Evaluation Results (n={num_evaluated}): "
                   f"Acc: {acc_all:.3f}, Comp: {comp_all:.3f}, "
                   f"NC: {nc_all:.3f}, "
                   f"Acc_med: {acc_all_med:.3f}, Comp_med: {comp_all_med:.3f}, "
                   f"NC_med: {nc_all_med:.3f}")
        logger.info("=" * 80)
        
        eval_dict = {
            "acc_med": acc_all_med,
            "comp_med": comp_all_med,
            "nc_med": nc_all_med,
        }
    else:
...
def main():
    ...
    if accelerator.process_index % 2 == 0:
        distill_gpu = True
    else:
        distill_gpu = False

    if distill_gpu:
        teacher_model = Pi3Teacher.from_pretrained("yyfz233/Pi3").to(accelerator.device).eval()
        logger.info(f"GPU {accelerator.process_index}: Loaded teacher model for distillation", main_process_only=False)
    else:
        teacher_model = None
        logger.info(f"GPU {accelerator.process_index}: No teacher model (no-distill GPU)", main_process_only=False)
    ...
    if args.pretrained_model_name_or_path is not None:
        dage_model = eval(args.model.name).from_pretrained(args.pretrained_model_name_or_path, strict=False, model_config=args.model.config)
    else:
        logger.info(f"Initializing model {args.model.name} with config: {args.model.config}")
        dage_model = eval(args.model.name)(**args.model.config)
    ...
    for name, param in dage_model.named_parameters():
    ...
    num_trainable = sum(p.numel() for p in dage_model.parameters() if p.requires_grad)
    num_total = sum(p.numel() for p in dage_model.parameters())
    ...
    if accelerator.is_main_process:
        ...
        ema_model = torch.optim.swa_utils.AveragedModel(dage_model, device=accelerator.device, avg_fn=ema_avg_fn)
    ...
    dage_model, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        dage_model, optimizer, train_dataloader, lr_scheduler
    )
    ...
    if accelerator.mixed_precision == "fp16":
        ...
        # dage_model.to(dtype=weight_dtype)
    ...
    if accelerator.process_index == 0 or accelerator.process_index == 1:
        ...
        tracker_config["tasks1"] = "_".join(tracker_config["tasks1"])
        tracker_config["tasks2"] = "_".join(tracker_config["tasks2"])
    ...
    acts_tea = {}
    if distill_gpu:
        # Only hook teacher, not student (student features will come from model output)
        mapping_layers_tea = ["decoder.33"]

        add_hook(teacher_model, acts_tea, mapping_layers_tea)

    ######################################

    loss_function = Pi3DistillLoss(train_conf=False, use_edge_loss=use_edge_loss)
    loss_function_no_distill = Pi3Loss(train_conf=False, use_edge_loss=use_edge_loss)
    ...
    for step, batch in enumerate(train_dataloader):
        with accelerator.accumulate(dage_model):   
            ...
            if distill_gpu:
                acts_tea.clear()
                teacher_max_size = 672
                teacher_patch_size = 14*2

                if original_width > original_height:
                    teacher_width = min(teacher_max_size, original_width // teacher_patch_size * teacher_patch_size) # NOTE hardcode here 
                    teacher_height = int((teacher_width / original_aspect_ratio) // teacher_patch_size * teacher_patch_size)
                else:
                    teacher_height = min(teacher_max_size, original_height // teacher_patch_size * teacher_patch_size) # NOTE hardcode here 
                    teacher_width = int((teacher_height * original_aspect_ratio) // teacher_patch_size * teacher_patch_size)

                teacher_prior_resolution = (teacher_height, teacher_width)
                student_prior_resolution = (teacher_height//2, teacher_width//2)

                with accelerator.autocast():
                    with torch.no_grad():
                        teacher_pred_dict = teacher_model(
                            rgb_images,
                            return_precomputed_hidden=False,
                            prior_resolution=teacher_prior_resolution,
                        )

                    pred_dict = dage_model(
                        rgb_images,
                        lr_resolution=student_prior_resolution,
                        hr_num_tokens=int((original_height // 14) * (original_width // 14)),
                    )
                    
                    # Extract student features for distillation
                    student_feat = pred_dict.get('student_feat_for_distill', None)
                    # Create acts_stu dict to match expected interface
                    acts_stu = {'student_feat': student_feat} if student_feat is not None else {}
                    mapping_layers_stu = ['student_feat'] if student_feat is not None else []

                with torch.cuda.amp.autocast(enabled=True, dtype=torch.float32):
                    loss, loss_dict = loss_function(
                        pred=pred_dict,
                        pred_teacher=teacher_pred_dict,
                        gt_raw=batch,
                        device=accelerator.device,
                        use_moge=use_moge,
                        acts_tea=acts_tea,
                        acts_stu=acts_stu,
                        mapping_layers_tea=mapping_layers_tea,
                        mapping_layers_stu=mapping_layers_stu,
                        use_real_loss=use_real_loss,
                        camera_loss_weight=camera_loss_weight,
                        kd_alpha=1.0,
                        feat_distill_type=feat_distill_type,
                        metric_scale_loss=metric_scale_loss
                    )

                # Clean up teacher outputs to free memory immediately after loss computation
                del teacher_pred_dict
            else:
                prior_max_size = 336
                prior_patch_size = 14
                ...
                # num_tokens = accelerate.utils.broadcast_object_list([random.randint(1200, 3600)])[0]
                # num_tokens = random.randint(1600, 3600)
                num_tokens = min(3600, (original_height // 14) * (original_width // 14))
                ...
                with torch.cuda.amp.autocast(enabled=True, dtype=torch.float32):
                    loss, loss_dict = loss_function_no_distill(
                        pred=pred_dict,
                        gt_raw=batch,
                        device=accelerator.device,
                        use_moge=use_moge,
            ...
            if accelerator.sync_gradients:
                accelerator.clip_grad_norm_(dage_model.parameters(), args.max_grad_norm)
        ...
        if accelerator.sync_gradients:
            ...
            # EMA update            
            if accelerator.is_main_process:
                ema_model.update_parameters(dage_model)
            ...
            if global_step % args.checkpointing_steps == 0:
                ...
                if accelerator.is_main_process:
                    # Run multi-view reconstruction evaluation before saving checkpoint
                    
                    # Use EMA model for evaluation
                    try:
                        mv_recon_eval_dict = evaluate_mv_recon(
                        ...
                        if mv_recon_eval_dict is not None:
                            accelerator.log({
                                f"eval/mv_recon_nrgbd_{key}": value for key, value in mv_recon_eval_dict.items()
                            }, step=global_step)
                    except Exception as e:
                        logger.warning(f"Error evaluating multi-view reconstruction: {e}")
                    ...
                    torch.save({
                        "model": unwrap_model(dage_model, accelerator).state_dict(),
                        "global_step": global_step,
                        "config": args.model.config,
                    }, model_save_path)
    ...
    if accelerator.is_main_process:
        unwrapped_model = unwrap_model(dage_model, accelerator)
        # Save the fine-tuned MoGe model weights for later reuse
        model_save_path = os.path.join(args.output_dir, "dage_model.pt")
        torch.save({
