...
def recover_focal_shift_same(points: torch.Tensor, mask: torch.Tensor = None, focal: torch.Tensor = None, downsample_size: Tuple[int, int] = (64, 64)):
    """
    Recover the depth map and FoV from a point map with unknown z shift and focal.
    ...
    for i in range(points.shape[0]):
        ...
        uv_lr_list.append(uv_lr_i_np)
        points_lr_list.append(points_lr_i_np)

    uv_lr_list = np.concatenate(uv_lr_list, axis=0)
    points_lr_list = np.concatenate(points_lr_list, axis=0)

    # optim_shift, optim_focal = solve_optimal_focal_shift(uv_lr_list, points_lr_list)

    if focal is None:
        optim_shift, optim_focal = solve_optimal_focal_shift(uv_lr_list, points_lr_list)
        optim_shift = [float(optim_shift)] * points.shape[0]
        optim_focal = [float(optim_focal)] * points.shape[0]
        # optim_focal.append(float(optim_focal_i))
    else:
        optim_shift = solve_optimal_shift(uv_lr_list, points_lr_list, focal_np[0])
        optim_shift = [float(optim_shift)] * points.shape[0]
...
@torch.no_grad()
def refine_points_with_focal_shift(
    points,
    point_mask,
):
    original_height, original_width = points.shape[-3:-1] # points can have shape B,T,H,W,3 or T,H,W,3
    # area = original_height * original_width
    aspect_ratio = original_width / original_height
    # Get camera-space point map. (Focal here is the focal length relative to half the image diagonal)
    if not (point_mask > 0.5).any():
        return points

    try:
        focal, shift = recover_focal_shift(points, point_mask > 0.5)
