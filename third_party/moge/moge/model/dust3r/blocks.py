...
class DecoderBlockCombined(nn.Module):
    ...
    def forward(self, x, y, z, xpos, ypos, zpos):
        x = x + self.drop_path(self.attn(self.norm1(x), xpos))


        z_ = self.norm_z(z)
        x = x + self.drop_path(self.cross_attn2(self.norm4(x), z_, z_, xpos, zpos))
        x = x + self.drop_path(self.mlp2(self.norm5(x)))
