...
class DINOv2Encoder(nn.Module):
    ...
    def forward(self, image: torch.Tensor, token_rows: int, token_cols: int, return_class_token: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if image.shape[-2] != token_rows * 14 or image.shape[-1] != token_cols * 14:
...
class DINOv2EncoderLightweight(nn.Module):
    "Wrapped DINOv2 encoder supporting gradient checkpointing. Input is RGB image in range [0, 1]."
    backbone: DinoVisionTransformer
    image_mean: torch.Tensor
    image_std: torch.Tensor
    ...
    def __init__(self, backbone: str, intermediate_layers: Union[int, List[int]], dim_out: int, **deprecated_kwargs):
        super(DINOv2EncoderLightweight, self).__init__()
        ...
        # NOTE hardcode reduce number of blocks
        self.backbone.blocks = self.backbone.blocks[:5]
        ...
        # self.output_projection = nn.Conv2d(in_channels=self.dim_features, out_channels=dim_out, kernel_size=1, stride=1, padding=0,)
    ...
    def forward(self, image: torch.Tensor, token_rows: int, token_cols: int, return_class_token: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        features = self.backbone.get_intermediate_layers(image_14, n=self.intermediate_layers, return_class_token=True)
        # breakpoint()
        # features = self.backbone.get_intermediate_layers(image_14, n=1, return_class_token=True)
        ...
        #         for proj, (feat, clstoken) in zip(self.output_projections, features)
        # ], dim=1).sum(dim=1)                    
        # x = self.output_projection(features[-1][0].permute(0, 2, 1).unflatten(2, (token_rows, token_cols)).contiguous())

        x = features[-1][0].permute(0, 2, 1).unflatten(2, (token_rows, token_cols)).contiguous()
        if return_class_token:
            return x, features[-1][1]
        else:
            return x
...
class DINOv2PatchEmbed(nn.Module):
    ...
    def __init__(self, backbone: str, intermediate_layers: Union[int, List[int]], dim_out: int, **deprecated_kwargs):
        super(DINOv2PatchEmbed, self).__init__()
    ...
    def forward(self, image: torch.Tensor, token_rows: int, token_cols: int, return_class_token: bool = False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Get intermediate layers from the backbone
        features = self.backbone.get_patch_embed(image_14, return_class_token=True)
        # breakpoint()
        # features = self.backbone.get_intermediate_layers(image_14, n=1, return_class_token=True)
