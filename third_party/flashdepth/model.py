...
class FlashDepth(nn.Module):
    ...
    def dpt_features_skip_mamba(self, input_shape, dpt_features, in_dpt_layer):
        # reshape to (B, T*h*w, c) for mamba
        # if len(input_shape)==4:
        #     B, C, H, W = input_shape
        #     T = 1
        ...
        # mamba_out = original_dpt_features

       
        return dpt_features

    def get_dpt_features(self, x, input_shape=None, mamba_fn=None):
        if mamba_fn is None:
        ...
        if self.hybrid_configs is None:
            ...
            if self.use_mamba:
                out = self.depth_head.forward_with_mamba(intermediate_features, patch_h, patch_w, temporal_layer=self.mamba_in_dpt_layer, mamba_fn=mamba_fn, shape_placeholder=input_shape)
            else:
            ...
            out = self.depth_head.forward_with_mamba(student_intermediate_features, patch_h, patch_w, temporal_layer=self.mamba_in_dpt_layer, mamba_fn=mamba_fn, shape_placeholder=input_shape,
    ...
    @torch.no_grad()
    def infer(self, video, use_mamba=True, max_size=None, skip_mamba=False, **kwargs):
        ...
        batch_size, num_frames, _, original_height, original_width = video.shape
        ...
        video = F.interpolate(video.reshape(batch_size*num_frames, _, original_height, original_width), (new_height, new_width), mode='bilinear', antialias=True).clamp(0, 1)
        video = video.reshape(batch_size, num_frames, _, new_height, new_width)


        image_mean = torch.tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1).to(device)
        image_std = torch.tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1).to(device)


        video = (video - image_mean) / image_std
        ...
        for i in range(video.shape[1]):
            ...
            # dpt_features = self.get_dpt_features(frame)
            # print(f"frame shape: {frame.shape}, B: {B}, C: {C}, H: {H}, W: {W}, new_height: {new_height}, new_width: {new_width}")
            # print(f"patch_h: {patch_h}, patch_w: {patch_w}")
            if skip_mamba:
                dpt_features = self.get_dpt_features(frame, input_shape=(B,C,H,W), mamba_fn=self.dpt_features_skip_mamba)
            else:
                dpt_features = self.get_dpt_features(frame, input_shape=(B,C,H,W), mamba_fn=self.dpt_features_to_mamba)
        ...
        preds = F.interpolate(rearrange(preds, 'b t h w -> (b t) 1 h w'), (original_height, original_width), mode='bilinear', antialias=True)
        preds = rearrange(preds, '(b t) 1 h w -> b t h w', b=batch_size).squeeze(0)
