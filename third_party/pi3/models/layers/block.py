...
class BlockRope(nn.Module):
    ...
    def forward(self, x: Tensor, xpos=None, **kwargs) -> Tensor:
        if kwargs.get('return_attn_weight', False) is True:
            def attn_residual_func(x: Tensor) -> Tensor:
                x, attn_weight = self.attn(self.norm1(x), xpos=xpos, return_attn_weight=True)
                return self.ls1(x), attn_weight

            def ffn_residual_func(x: Tensor) -> Tensor:
                return self.ls2(self.mlp(self.norm2(x)))

            attn_x, attn_weight = attn_residual_func(x)
            x = x + attn_x
            x = x + ffn_residual_func(x)
            ...
            def attn_residual_func(x: Tensor) -> Tensor:
                # if tpos is not None:
                #     return self.ls1(self.attn(self.norm1(x), xpos=xpos, tpos=tpos))
                # elif global_merging is not None:
                ...
                return self.ls1(self.attn(self.norm1(x), xpos=xpos, **kwargs))
...
class CrossBlockRope(nn.Module):
    def __init__(
        ...
        # NOTE when rope_interpolated is not None, use the interpolated rope for the self-attention
        if rope_interpolated is None:
            self.attn = attn_class(
                dim,
                num_heads=num_heads,
                qkv_bias=qkv_bias,
    ...
    def forward(self, x: Tensor, y: Tensor, xpos=None, ypos=None, real_xpos=None) -> Tensor:
        # Assert that xpos, ypos, and real_xpos are either all None or all not None
        assert (xpos is None and ypos is None and real_xpos is None) or \
               (xpos is not None and ypos is not None and real_xpos is not None), \
               "xpos, ypos, and real_xpos must be either all None or all not None"
        
        def attn_residual_func(x: Tensor) -> Tensor:
            return self.ls1(self.attn(self.norm1(x), xpos=real_xpos))
...
class CrossOnlyBlockRope(nn.Module):
    ...
    def zero_init(self):
        ...
        self.mlp.fc2.bias.data.zero_()
        self.cross_attn.proj.weight.data.zero_()
        self.cross_attn.proj.bias.data.zero_()
        # self.cross_first = cross_first
