...
class GaussianSplatRenderer(nn.Module):
    ...
    def prepare_splats(self, gs_points, extrinsics, images, gs_params, context_nums, target_nums, context_predictions=None, position_from="gsdepth+predcamera", debug=False):
        """
        ...
        splats["quats"] = reg_dense_rotation(quats.reshape(B, S * H * W, 4))
        splats["scales"] = reg_dense_scales(scales.reshape(B, S * H * W, 3)).clamp_max(0.3)
        splats["opacities"] = reg_dense_opacities(opacities.reshape(B, S * H * W))
        residual_sh = reg_dense_sh(residual_sh.reshape(B, S * H * W, self.nums_sh * 3))
        ...
        global_gs_points = torch.einsum('bnij, bnhwj -> bnhwi', extrinsics[:, :S], homogenize_points(gs_points))[..., :3]
